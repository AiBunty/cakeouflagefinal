...
function media_offer_visibility_status(array $topOffer, bool $hasTopOfferRow, array $couponOptions): array
{
  ...
  $pageScope = (string)($topOffer['page_scope'] ?? 'all_pages');
  $linkedCouponId = (int)($topOffer['linked_coupon_id'] ?? 0);
  if ($linkedCouponId <= 0) {
    $scopeSuffix = $pageScope === 'exclude_checkout_auth'
      ? ' Checkout and auth pages are intentionally excluded.'
      : ' Banner appears on all public pages.';
    return array(
      'state' => 'active',
      'reason' => 'Visible now. No coupon is linked, so this runs as a message-only campaign.' . $scopeSuffix,
    );
  }
  ...
  return array(
    'state' => 'active',
    'reason' => 'Visible now. Schedule and linked coupon are both currently valid.' . ($pageScope === 'exclude_checkout_auth' ? ' Checkout and auth pages are intentionally excluded.' : ''),
  );
}
...
<div class="media-shell" data-max-upload-bytes="<?= (int)$effectiveUploadLimitBytes ?>">
  ...
  <div class="media-grid">
    ...
    <div class="media-card">
      ...
      <div class="media-card__body">
        <p class="media-help">The top offer banner is now managed from the <a href="coupons.php" style="color:#80001F;font-weight:700;">Coupons page</a>. Open any coupon and click <em>Set as Top Banner</em> to configure and activate the site-wide offer strip.</p>
      </div>
    </div>
  </div>
</div>
