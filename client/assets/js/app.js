const BASE = "/Cakeouflage-E-commerce";
document.addEventListener("DOMContentLoaded", () => {
  ...
  const initCheckout = async () => {
    ...
    const updateCheckoutQr = (amount) => {
      const enc = encodeURIComponent;
      const upiData = `upi://pay?pa=${upiId}&pn=Cakeouflage&am=${amount}&cu=INR`;
      if (upiQR) upiQR.src = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${enc(upiData)}`;
      if (upiAmountText) upiAmountText.textContent = `Pay ₹${amount}`;
      if (upiDeepLink) { upiDeepLink.href = upiData; upiDeepLink.style.display = "flex"; }
      if (upiButtonHint) upiButtonHint.style.display = "block";
    };
  ...
  const renderWishlistCards = (items = [], compact = false) => {
    ...
    return items
      .map((item) => {
        ...
        return `
        <article class="product-card">
          <a class="product-card__image-wrap${item.is_chef_special ? ' is-chefs-special' : ''}" href="${BASE}/product/${item.slug}">
          ...
          <div class="product-card__actions">
            <a class="btn btn--secondary" href="${BASE}/product/${item.slug}">View</a>
  ...
  const initAccount = async () => {
    ...
    try {
      ...
      if (profileDoa) {
        profileDoa.value = profile.anniversary_date || profile.doa || "";
      }
    ...
    container.addEventListener("click", async (event) => {
      ...
      try {
        ...
        drawerBody.innerHTML = `
          ...
          <h4>Timeline</h4>
          <ul>
            ${timeline.map((step) => `<li>${step.step} - ${step.timestamp || "-"}</li>`).join("")}
          </ul>
        `;
        openDrawer();
      } catch (error) {
  ...
  const initAuthForms = () => {
    const sendOtp = async (email) => {
      return utils.apiPost("/api/send-otp", { email });
    };

    const verifyOtp = async (payload) => {
      return utils.apiPost("/api/verify-otp", payload);
    };

    const loginPage = document.querySelector('[data-page="login"]');
    if (loginPage) {
      const loginForm = document.getElementById("loginForm");
      if (loginForm?.dataset.otpManaged === "1") {
        return;
      }
      ...
      loginSendOtpBtn?.addEventListener("click", async () => {
        ...
        if (!email) {
          loginStatus.textContent = "Please enter your email first.";
          return;
        }
    ...
    if (registerPage) {
      ...
      if (registerForm?.dataset.otpManaged === "1") {
        return;
      }
      ...
      registerSendOtpBtn?.addEventListener("click", async () => {
        ...
        if (!email) {
          registerStatus.textContent = "Please enter your email first.";
          return;
        }

        try {
          registerSendOtpBtn.disabled = true;
          await sendOtp(email);
          registerStatus.textContent = "OTP sent to your email.";
          window.setTimeout(() => {
            registerSendOtpBtn.disabled = false;
          }, 30000);
        } catch (error) {
      ...
      registerForm?.addEventListener("submit", async (event) => {
        ...
        if (!fullName || !phone || !email || !otp) {
          registerStatus.textContent = "Name, phone, email, and OTP are required.";
          return;
        }
  ...
  const initCustomCakeInquiry = () => {
    ...
    const submitBtn = document.getElementById("customCakeInquirySubmitBtn");
    const countryCodeInput = document.getElementById("cciCountryCode");
    const phoneInput = document.getElementById("cciPhone");
    ...
    const isValidPhone = (countryCode, phoneRaw) => {
    ...
    phoneInput?.addEventListener("input", () => {
      const cleaned = String(phoneInput.value || "").replace(/\D+/g, "");
      phoneInput.value = cleaned;
      if (countryCodeInput?.value === "+91" && cleaned.length > 10) {
    ...
    form?.addEventListener("submit", async (event) => {
      ...
      const formData = new FormData(form);
      const countryCode = String(formData.get("phone_country_code") || "").trim();
      const phone = String(formData.get("phone") || "").trim();

      if (!isValidPhone(countryCode, phone)) {
        const phoneMsg = countryCode === "+91"
          ? "For India (+91), mobile number must be exactly 10 digits."
      ...
      if (!String(formData.get("privacy_consent") || "").trim()) {
        setStatus("Please accept the Privacy Policy to continue.", true);
        return;
      }
      ...
      try {
        const response = await fetch("/api/inquiries/custom-cake", {
          method: "POST",
        ...
        if (!response.ok || !payload || payload.success === false) {
          throw new Error(payload?.message || "Could not submit your request right now.");
        }

        setStatus("Build Your Own Cake request submitted successfully. Our team will share your custom quote shortly.");
        form.reset();
        if (countryCodeInput) {
          countryCodeInput.value = "+91";
        }
      } catch (error) {
        setStatus(error.message || "Could not submit your request right now.", true);
      } finally {
        if (submitBtn) {
  ...
  const initByocQuoteAccept = () => {
    const page = document.querySelector('[data-page="byoc-quote-accept"]');
    if (!page) {
      return;
    }

    const form = document.getElementById("byocQuoteAcceptForm");
    const status = document.getElementById("byocQuoteAcceptStatus");
