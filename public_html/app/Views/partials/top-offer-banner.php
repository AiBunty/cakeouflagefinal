...
<style>
  ...
  .site-top-offer__copy:hover,
  .site-top-offer__cta:hover {
    background: #fff;
    color: #7c1227;
  }
...
<section class="site-top-offer" aria-label="Limited time offer">
  <div class="site-top-offer__inner">
    ...
    <?php endif; ?>
    <?php if ($offerCode !== ''): ?>
      <span class="site-top-offer__code">Code: <?= htmlspecialchars($offerCode, ENT_QUOTES, 'UTF-8'); ?></span>
      <button type="button" class="site-top-offer__copy" data-copy-code="<?= htmlspecialchars($offerCode, ENT_QUOTES, 'UTF-8'); ?>">Copy code</button>
    <?php endif; ?>
    <a href="<?= htmlspecialchars($offerCtaUrl, ENT_QUOTES, 'UTF-8'); ?>" class="site-top-offer__cta"><?= htmlspecialchars($offerCtaLabel, ENT_QUOTES, 'UTF-8'); ?></a>
  </div>
</section>
<?php if ($offerExpiryAt !== ''): ?>
<script>
(function () {
  ...
  const formatCountdown = (remainingMs) => {
    ...
    if (hours >= 24) {
      ...
      return { text: `Ends in ${days}d ${String(hours % 24).padStart(2, '0')}h`, expired: false };
    }
    if (hours >= 1) {
      return { text: `Ends in ${hours}h ${String(minutes).padStart(2, '0')}m`, expired: false };
    }
