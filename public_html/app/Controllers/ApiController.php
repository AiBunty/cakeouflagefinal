...
$customerPhone = $customerPhone;
        ...
        if (($fulfilmentMode === 'delivery' || $fulfilmentMode === 'custom_delivery') && ($deliveryDate === '' || $slotId <= 0)) {
            Response::json([
                'success' => false,
...
INSERT INTO orders (
    ...
    delivery_postal_code, delivery_distance_km, delivery_fee,
    subtotal, discount_total, tax_total, grand_total
) VALUES (
    :order_number, :user_id, :customer_name, :customer_email, :customer_phone,
    :fulfilment_mode, "pending", "pending", :payment_method,
    :scheduled_slot, :scheduled_slot_label,
    :postal_code, :distance_km, :delivery_fee,
    :subtotal, :discount_total, 0, :grand_total
)');
...
$orderStmt->execute([
    ...
    'grand_total' => max(0, (float)($cart['subtotal'] ?? 0) - (float)($cart['discount_total'] ?? 0)),
]);
    ...
    public function customCakeInquirySubmit(): void
{
    ...
    try {
        ...
        $crmContext['inquiry_id'] = $inquiryId;
    ...
    public function customCakeQuoteAccept(string $token): void
    {
        ...
        try {
            ...
            $inquiryUpdate = $pdo->prepare('UPDATE inquiries SET status = "closed", updated_at = NOW() WHERE id = :id AND inquiry_type = "custom_cake"');
            ...
            Response::json([
                'success' => true,
                'message' => 'Quote accepted. Your order is now created for payment processing.',
                'data' => [
                    'order_id' => $orderId,
                    'order_number' => $orderNumber,
