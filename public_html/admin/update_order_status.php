...
// Build redirect back to orders page with flash
$flashParam = 'order_updated=' . $id . '&_status=' . urlencode($status);
    ...
    if (is_array($parts) && isset($parts['query'])) {
        ...
        if ($query !== '') {
            ...
            if (is_array($params)) {
                ...
                if (!empty($params)) {
                    $flashParam = http_build_query($params) . '&order_updated=' . $id;
                }
            }
        }
    }
...
header('Location: /admin/orders.php?' . $flashParam);
exit;