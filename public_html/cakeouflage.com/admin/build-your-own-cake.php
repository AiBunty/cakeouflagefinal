...
if (!function_exists('queue_custom_cake_comm_log')) {
    ...
    function queue_custom_cake_comm_log($conn, $channel, $eventKey, $recipient, array $context)
    {
        ...
        $jobStmt = $conn->prepare('INSERT INTO queue_jobs (job_type, payload_json, status, available_at, attempts) VALUES ("communication_send", ?, "queued", NOW(), 0)');
        if (!$jobStmt) {
...
$tplStmt = $conn->prepare('INSERT INTO communication_templates (channel, event_key, subject, body_template, is_active) VALUES ("email", "build_your_cake_quote_email", ?, ?, 1) ON DUPLICATE KEY UPDATE subject = VALUES(subject), body_template = VALUES(body_template), is_active = 1');
if ($tplStmt) {
...
if (!empty($rowIds)) {
  $types = str_repeat('s', 2) . str_repeat('i', count($rowIds));
  $params = [
    'build_your_cake_quote_email',
    'build_your_cake_quote_whatsapp',
  ];
...
<div class="byoc-shell">
  ...
  <div class="byoc-card">
    <form method="get" class="byoc-filters">
          ...
          <tr>
            ...
            <td>
              <form method="post" class="byoc-grid" data-send-form="<?= (int)$row['id'] ?>">
                ...
                <textarea name="quote_message" placeholder="Custom quote message (managed template fallback context)" required></textarea>
                <div>
                  <button type="submit" class="byoc-btn">Send Quote (Email + WhatsApp)</button>
                </div>
              </form>
