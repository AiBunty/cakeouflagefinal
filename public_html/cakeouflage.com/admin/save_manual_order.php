...
try {
    ...
    $orderInsert = $conn->prepare('INSERT INTO orders (order_number, user_id, customer_name, customer_email, customer_phone, fulfilment_mode, order_status, payment_status, payment_method, payment_confirmed_at, payment_confirmed_by_admin_id, scheduled_slot, scheduled_slot_label, billing_address_line1, billing_address_line2, billing_city, billing_state, billing_postal_code, subtotal, discount_total, tax_total, grand_total, admin_note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, ?, ?)');
    ...
    $orderInsert->bind_param('sissssssssisssssssdds', $orderNumber, $userId, $customerName, $customerEmail, $customerPhone, $fulfilmentMode, $orderStatus, $paymentStatus, $paymentMethod, $confirmedAt, $confirmedBy, $scheduledSlotForDb, $scheduledSlotLabel, $billingAddressLine1, $billingAddressLine2, $billingCity, $billingState, $billingPostalCode, $amount, $amount, $finalNote);
    $orderInsert->execute();
