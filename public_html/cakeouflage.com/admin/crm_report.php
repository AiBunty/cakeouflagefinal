...
if (in_array($export, array('excel', 'csv', 'pdf'), true)) {
  ...
  if ($export === 'csv') {
    ...
    header('Content-Disposition: attachment; filename="crm_users_export.csv"');
    $out = fopen('php://output', 'w');
    fputcsv($out, array('Name', 'Phone', 'Email', 'Orders', 'Completed', 'Pending'));
    foreach ($exportUsers as $u) {
      fputcsv($out, array(
        ...
        (string)($u['email'] ?? ''),
        (int)($u['order_count'] ?? 0),
        (int)($u['completed_count'] ?? 0),
        (int)($u['pending_count'] ?? 0),
      ));
    }
    fclose($out);
    exit;
  ...
  <?php if ($selectedSubReport === 'users'): ?>
    ...
    <div class="crm-report-panel__body">
      <form class="crm-report-form" method="get">
        ...
        <button class="crm-report-btn crm-report-btn--ghost" type="submit" name="export" value="csv">Download CSV</button>
        <button class="crm-report-btn crm-report-btn--ghost" type="submit" name="export" value="excel">Download Excel</button>
