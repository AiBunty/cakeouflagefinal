...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        ...
        if ($action === 'toggle_coupon_banner') {
          $couponId = (int)($_POST['coupon_id'] ?? 0);
          $nextState = (int)($_POST['next_state'] ?? 0) === 1 ? 1 : 0;
          if ($couponId > 0) {
            $bannerLookup = $conn->prepare('SELECT id, linked_coupon_id, is_active FROM banners WHERE placement = "site_top_offer" ORDER BY id DESC LIMIT 1');
            if ($bannerLookup) {
              $bannerLookup->execute();
              $bannerRow = $bannerLookup->get_result()->fetch_assoc();
              $bannerLookup->close();

              if (!$bannerRow) {
                $messageType = 'error';
                $message = 'No top offer banner exists yet. Configure it in Media Center first.';
              } elseif ((int)($bannerRow['linked_coupon_id'] ?? 0) !== $couponId) {
                $messageType = 'error';
                $message = 'This coupon is not currently linked to the top offer banner.';
              } else {
                $bannerId = (int)$bannerRow['id'];
                $updateBanner = $conn->prepare('UPDATE banners SET is_active = ?, updated_at = NOW() WHERE id = ? LIMIT 1');
                if ($updateBanner) {
                  $updateBanner->bind_param('ii', $nextState, $bannerId);
                  $updateBanner->execute();
                  $updateBanner->close();
                  $message = $nextState === 1 ? 'Coupon banner activated.' : 'Coupon banner deactivated.';
                }
              }
            }
          }
...
$topOfferStmt = $conn->prepare('SELECT linked_coupon_id, is_active, starts_at, ends_at FROM banners WHERE placement = "site_top_offer" ORDER BY id DESC LIMIT 1');
if ($topOfferStmt) {
          ...
          <tr>
            ...
            <td>
              <div class="coupon-actions" style="margin-top:0; justify-content:flex-start;">
                ...
                <form method="post" style="margin:0">
                  <input type="hidden" name="action" value="toggle_coupon_banner">
                  <input type="hidden" name="coupon_id" value="<?= $couponId ?>">
                  <input type="hidden" name="next_state" value="<?= (int)($topOfferLink['is_active'] ?? 0) === 1 ? '0' : '1' ?>">
                  <button class="coupon-btn ghost" type="submit"><?= (int)($topOfferLink['is_active'] ?? 0) === 1 ? 'Deactivate Banner' : 'Activate Banner' ?></button>
                </form>
                <?php endif; ?>
