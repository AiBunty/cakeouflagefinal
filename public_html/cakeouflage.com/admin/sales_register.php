...
$allowedPaymentStatus = ['paid', 'pending', 'failed', 'refunded', 'credit'];
...
$totalsResult = $conn->query("SELECT
  ...
  FROM orders o
  WHERE $whereSql");
$totals = $totalsResult ? $totalsResult->fetch_assoc() : ['cash_total' => 0, 'bank_total' => 0, 'credit_total' => 0, 'overall_total' => 0];
    ...
    <div class="sr-body">
      ...
      <div class="sr-table-wrap">
        <table class="sr-table">
          ...
          <tbody>
            <?php if (empty($rows)): ?>
              <tr><td colspan="9" class="sr-muted">No orders found for selected filters.</td></tr>
            <?php else: ?>
              <?php foreach ($rows as $row): ?>
                <?php
                  $statusClass = 'ok';
                  if ((string)$row['payment_status'] === 'pending' || (string)$row['payment_status'] === 'credit') {
                    $statusClass = 'pending';
                  }
