...
final class AdminApiController
{
    ...
    public function productsList(): void
    {
        ...
        $sql = 'SELECT p.id, p.name, p.slug, p.sku, p.short_description, p.long_description,
                   ...
                   p.availability_status, p.is_featured, p.is_bestseller,
        ...
        try {
            ...
            $stmt = $pdo->prepare(
                'INSERT INTO products (
                    ...
                    collection_category_id, subcategory_id, occasion_tag, dietary_tag,
                    ...
                    is_featured, is_bestseller, seo_title, seo_description,
    ...
    public function ordersExportCsv(): void
    {
        ...
        header('Content-Disposition: attachment; filename="cakeouflage-orders-export-' . date('Ymd-His') . '.csv"');
    ...
    public function bulkTemplate(): void
    {
        ...
        $content = [
            ['product_name', 'category_slug', 'description', 'price', 'discount_price', 'sku', 'stock', 'tags', 'variant_info', 'image_url'],
            ['Belgian Velvet Truffle', 'classic-cakes', 'Dark chocolate premium sponge', '799', '749', 'CKF-NEW-001', '25', 'bestseller|eggless', '0.5 kg:549|1 lb:799|1.5 lb:1099|2 lb:1399|2.5 lb:1799|3 lb:2199', '/client/assets/images/placeholder-cake-1.jpg'],
            ['Raspberry Cocoa Bloom', 'birthday-cakes', 'Raspberry and cocoa layered cake', '999', '', 'CKF-NEW-002', '18', 'featured|custom', '0.5 kg:699|1 lb:999|1.5 lb:1299|2 lb:1699|2.5 lb:2099|3 lb:2499', '/client/assets/images/placeholder-cake-2.jpg'],
        ];

        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename="cakeouflage-product-template.csv"');
        $out = fopen('php://output', 'wb');
        if ($out !== false) {
    ...
    public function productsExportCsv(): void
    {
        ...
        $sql = 'SELECT
                ...
                WHERE p.deleted_at IS NULL
                GROUP BY p.id, collection.name, subcategory.name, p.name
        ...
        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename="cakeouflage-products-export-' . date('Ymd-His') . '.csv"');
        ...
        fputcsv($out, ['Category', 'Subcategory', 'Product Name', 'Variants']);
    ...
    public function bulkImportProducts(): void
    {
        ...
        if (!isset($_FILES['file']) || !is_array($_FILES['file'])) {
            Response::json(['success' => false, 'message' => 'CSV file is required under field "file"'], 422);
            return;
        }
        ...
        if ($extension !== 'csv') {
            Response::json(['success' => false, 'message' => 'Only CSV uploads are supported'], 422);
            return;
        }
        ...
        if ($handle === false) {
            Response::json(['success' => false, 'message' => 'Cannot read uploaded CSV'], 422);
            return;
        }
        ...
        while (($row = fgetcsv($handle)) !== false) {
            $lineNumber++;
            $record = $this->mapImportRow($row, $headerMap);
                ...
                if ($existingId > 0) {
                    ...
                    $insertStmt = $pdo->prepare(
                        'INSERT INTO products (
                            ...
                            dietary_tag, availability_status, lead_time_hours, delivery_eligible, pickup_eligible,
                            featured_image, starting_price, base_price, discount_price, stock_quantity,
                            is_featured, is_bestseller, is_b2b_enabled
                        ) VALUES (
                            :name, :slug, :short_description, :long_description, :sku, :collection_category_id,
                            :dietary_tag, :availability_status, 24, 1, 1,
                            :featured_image, :starting_price, :base_price, :discount_price, :stock_quantity,
                            :is_featured, :is_bestseller, :is_b2b_enabled
                        )'
                    );
        ...
        if (count($failedRows) > 0) {
            $failedRowsCsvPath = preg_replace('/\.json$/', '.failed-rows.csv', $logFile);
            if (is_string($failedRowsCsvPath) && $failedRowsCsvPath !== '') {
                $csv = fopen($failedRowsCsvPath, 'wb');
                if ($csv !== false) {
                    fputcsv($csv, ['line', 'sku', 'reason']);
                    ...
                    fclose($csv);
                    $failedRowsCsvRelative = basename((string)$failedRowsCsvPath);
                }
            }
        }
    ...
    public function bulkImportFailedRowsCsv(string $file): void
    {
        ...
        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename="' . str_replace('.json', '-failed-rows.csv', $safeFile) . '"');
    ...
    public function mediaUpload(): void
    {
        ...
        $targetRelative = '/uploads/media/' . $yearMonth . '/' . $fileToken . '.jpg';
        $targetAbsolute = $this->projectRoot() . str_replace('/', DIRECTORY_SEPARATOR, $targetRelative);

        $optimized = $this->optimizeImageToJpeg($tmpName, $targetAbsolute, 1600, 82);
        if (!$optimized) {
    ...
    {
        return in_array($dietary, ['regular', 'eggless', 'vegan', 'sugar_free'], true) ? $dietary : 'regular';
    }
