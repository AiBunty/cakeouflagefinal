    ...
    <div class="checkout-layout">
      ...
      <form class="checkout-form" id="checkoutForm" novalidate>
        ...
        <div class="checkout-section card">
          ...
          <?php if ($isLoggedIn): ?>
          <div style="background:#f0fdf4; border:1px solid #bbf7d0; border-radius:10px; padding:10px 14px; margin-bottom:12px; font-size:0.88rem; color:#166534;">✅ Logged in — contact details pre-filled. You can edit if needed.</div>
          <?php endif; ?>
          <div class="form-row-2">
            <label class="form-control">
              <span class="form-label">Full Name <span class="form-required">*</span></span>
              <input id="customerName" name="customer_name" required autocomplete="name" placeholder="Priya Sharma" value="<?= $prefillName ?>">
            </label>
            <label class="form-control">
              <span class="form-label">Phone <span class="form-required">*</span></span>
             <input id="customerPhone" name="customer_phone" type="tel" autocomplete="tel" placeholder="+91 98765 43210" value="<?= $prefillPhone ?>">
            </label>
        ...
        <input id="customerEmail" name="customer_email" type="email" required autocomplete="email" placeholder="you@email.com" value="<?= $prefillEmail ?>">
          </label>
          <!-- 🔥 OTP UI START -->
<?php if (!$isLoggedIn): ?>
...
<!-- 🔥 OTP UI END -->
        ...
        <div class="checkout-section card" id="addressSection" style="display:none">
          ...
          <h2 class="checkout-section__title" style="margin-top:16px">Delivery Address</h2>
          <div class="form-row-2">
            <label class="form-control">
              <span class="form-label">Street / Area <span class="form-required">*</span></span>
              <input type="text" id="deliveryStreet" name="delivery_street" placeholder="House no., street, area, landmark" autocomplete="street-address">
            </label>
            <label class="form-control">
              <span class="form-label">Pincode <span class="form-required">*</span></span>
              <input type="text" id="deliveryPincode" name="delivery_pincode" placeholder="e.g. 422001" maxlength="6" inputmode="numeric" autocomplete="postal-code">
            </label>
          </div>
          <label class="form-control" style="margin-top:12px">
            ...
            <input type="url" id="deliveryMapsLink" name="delivery_maps_link" placeholder="https://maps.app.goo.gl/…">
          </label>
          <button type="button" class="btn btn--outline btn--sm" id="useMyLocationBtn" style="margin-top:12px">
            📍 Use My Location
          </button>
          <p class="form-hint" id="locationStatus" style="margin-top:6px;display:none"></p>
        </div>
        ...
        <div class="checkout-section card">
          ...
          <!-- Payment type: full vs advance -->
          <div style="margin-bottom:14px; padding:12px; background:#fff9f0; border:1px solid #fde68a; border-radius:10px;">
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Payment type toggle: full vs 50% advance
  const syncPaymentType = () => {
    const total = window.__checkoutGrandTotal || 0;
    if (!total) return;
    const type = form?.querySelector('input[name="payment_type"]:checked')?.value || 'full';
    const amt = type === 'advance_50' ? Math.ceil(total * 0.5) : total;
    document.getElementById('upiAmountText').textContent = type === 'advance_50'
      ? 'Pay ₹' + amt + ' now (50% advance). Balance ₹' + (total - amt) + ' due on delivery.'
      : 'Pay ₹' + amt + ' using UPI';
    generateQR(amt);
  };
  form?.querySelectorAll('input[name="payment_type"]').forEach(r => r.addEventListener('change', syncPaymentType));

  // Show/hide address section based on fulfilment mode
  ...
  form?.querySelectorAll('input[name="fulfilment_mode"]').forEach(r => r.addEventListener('change', syncAddressSection));
  syncAddressSection();

  // Use My Location (Nominatim reverse geocode)
  document.getElementById('useMyLocationBtn')?.addEventListener('click', function () {
    const statusEl = document.getElementById('locationStatus');
    if (!navigator.geolocation) {
      if (statusEl) { statusEl.textContent = 'Geolocation is not supported by your browser.'; statusEl.style.display = 'block'; }
      return;
    }
    if (statusEl) { statusEl.textContent = '📍 Detecting location…'; statusEl.style.display = 'block'; }
    navigator.geolocation.getCurrentPosition(
      async function (pos) {
        try {
          ...
          if (statusEl) { statusEl.textContent = '✅ Location detected. Please verify before placing order.'; statusEl.style.display = 'block'; }
        } catch (e) {
          if (statusEl) { statusEl.textContent = 'Could not detect address. Please enter it manually.'; statusEl.style.display = 'block'; }
        }
      },
      function () {
        if (statusEl) { statusEl.textContent = 'Location access denied. Please enter your address manually.'; statusEl.style.display = 'block'; }
      }
    );
  });
  ...
  if (!window.otpVerified) {
    ...
    // Validate + attach delivery address fields
    if (fulfilmentMode === 'delivery') {
      ...
      if (!street || !pincode) {
        alert('Please enter your delivery street address and pincode.');
        return;
      }
      formData.set('delivery_street', street);
      formData.set('delivery_pincode', pincode);
      const mapsLink = document.getElementById('deliveryMapsLink')?.value.trim() || '';
      if (mapsLink) formData.set('delivery_maps_link', mapsLink);
    }
