<?php
$base = '';
$contactPhoneDisplay = trim((string)($siteConfig['contact']['phone'] ?? ''));
$contactPhoneHref = (string)($siteConfig['contact']['phone_href'] ?? build_tel_href($contactPhoneDisplay));
$whatsappDigits = (string)($siteConfig['contact']['whatsapp_number'] ?? normalize_whatsapp_number((string)($siteConfig['contact']['whatsapp'] ?? $contactPhoneDisplay)));
...
$instagramUrl = (string)($siteConfig['social']['instagram_url'] ?? '');
$facebookUrl = (string)($siteConfig['social']['facebook_url'] ?? '');
$googleMapsUrl = (string)($siteConfig['contact']['google_maps_url'] ?? '');
...
$whatsappLink = (string)($siteConfig['contact']['whatsapp_link'] ?? build_whatsapp_link($whatsappDigits, 'Hi Cakeouflage! I would like to place a cake order.'));
?>
<div class="floating-contact-rail" aria-label="Quick contact actions">
  <?php if ($contactPhoneHref !== ''): ?>
  <a href="<?= htmlspecialchars($contactPhoneHref, ENT_QUOTES, 'UTF-8') ?>" class="floating-contact-rail__link floating-contact-rail__link--call" aria-label="Call Cakeouflage">
  ...
  <?php endif; ?>
  <?php if ($instagramUrl !== ''): ?>
  <a href="<?= htmlspecialchars($instagramUrl, ENT_QUOTES, 'UTF-8') ?>" class="floating-contact-rail__link floating-contact-rail__link--instagram" target="_blank" rel="noopener noreferrer" aria-label="Instagram Cakeouflage">
    <span class="floating-contact-rail__icon" aria-hidden="true">
  ...
  <a href="<?= htmlspecialchars($googleMapsUrl, ENT_QUOTES, 'UTF-8') ?>" class="floating-contact-rail__link floating-contact-rail__link--maps" target="_blank" rel="noopener noreferrer" aria-label="Open Cakeouflage location in Google Maps">
    <span class="floating-contact-rail__icon" aria-hidden="true">
      <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M12 2c4.41 0 8 3.59 8 8 0 5.49-7 12-8 12S4 15.49 4 10c0-4.41 3.59-8 8-8Zm0 4.5A3.5 3.5 0 1 0 12 13a3.5 3.5 0 0 0 0-7Z"/></svg>
    </span>
    <span class="floating-contact-rail__label">Location</span>
  </a>
  <?php endif; ?>
</div>
...
<footer class="site-footer" aria-label="Site Footer">
  ...
  <img src="<?= htmlspecialchars((string)($siteConfig['branding']['footer_logo_url'] ?? '/client/assets/images/whitelogo.png'), ENT_QUOTES, 'UTF-8') ?>" alt="<?= htmlspecialchars($brand['name'] ?? 'Cakeouflage', ENT_QUOTES, 'UTF-8') ?> Logo" onerror="<?= htmlspecialchars((string)($siteConfig['branding']['footer_logo_onerror'] ?? "this.onerror=null;this.src='/client/assets/images/whitelogo.png';"), ENT_QUOTES, 'UTF-8') ?>" style="height:140px;max-width:100%;width:auto;object-fit:contain;">

</div>
      <p class="site-footer__tagline"><?= htmlspecialchars((string)$brand['tagline'], ENT_QUOTES, 'UTF-8') ?></p>
      <div class="site-footer__social">
        <?php if ($instagramUrl !== ''): ?>
        <a href="<?= htmlspecialchars($instagramUrl, ENT_QUOTES, 'UTF-8') ?>" class="social-btn" target="_blank" rel="noopener noreferrer" aria-label="Instagram">
        ...
        <?php endif; ?>
        <?php if ($facebookUrl !== ''): ?>
        <a href="<?= htmlspecialchars($facebookUrl, ENT_QUOTES, 'UTF-8') ?>" class="social-btn" target="_blank" rel="noopener noreferrer" aria-label="Facebook">
